...
public class DBHandlerElementoOpcion {
    ...
    public void initData()
    {
        ...
        createOptions("TD","50","Cedula de Extranjería","5");
    ...
    public long insert(ElementoOpcion opcion)
    {
        ContentValues values = new ContentValues();
        values.put("opcGrupo", opcion.getGrupo());
        values.put("opcValor", opcion.getValor());
        values.put("opcTexto", opcion.getTexto());
        values.put("opcOrden", opcion.getOrden());

        long id = this.db.insert(this._tabla, null, values);
        return id;
    }


    public int createOptions(List<ElementoOpcion> opciones)
    {
        int recs = 0;
        this.db.beginTransaction();
        ...
        try
        {
            for (ElementoOpcion opcion : opciones)
            {
                long id = insert(opcion);
                recs++;
            }
            this.db.setTransactionSuccessful();
        }
    ...
    public long delete(String grupo)
    {
        String condition = "opcGrupo = ?";

        long id = 0;
        try
        {
            this.db.beginTransaction();
            id = this.db.delete(this._tabla, condition, new String[] {grupo});
            this.db.setTransactionSuccessful();
            this.db.endTransaction();
        }
        catch(Exception ex)
